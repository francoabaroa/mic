defmodule Mic.Chat.Anthropic do
  ...
  defp client do
    AWS.Client.create(
      Application.get_env(:mic, :access_key_id, ""),
      Application.get_env(:mic, :secret_access_key, ""),
      Application.get_env(:mic, :region, "")
    )
    |> AWS.Client.put_http_client({Mic.Chat.HTTPClient.StreamingFinch, [finch_name: Mic.Finch]})
  end
  ...
  def fix_messages(messages) when is_list(messages) do
    ...
    Enum.reduce(messages, [], fn msg, acc ->
      ...
      case should_insert do
        true -> [%Mic.Chat.Anthropic.Message{role: :assistant, content: ""} | [msg | acc]]
        false -> [msg | acc]
      end
    end)
  ...
  @spec convert_message(MicWeb.Message.t()) :: Mic.Chat.Anthropic.Message.t()
  def convert_message(%MicWeb.Message{sender: :user} = msg) do
    %Mic.Chat.Anthropic.Message{
      content: msg.content,
      role: :user
    }
  end
    ...
    streamhandler = fn
      ...
      {:data, data}, acc ->
        <<total_byte_length::binary-size(4), headers_byte_length::binary-size(4),
          _prelude_crc::binary-size(4), rest::binary>> = data
