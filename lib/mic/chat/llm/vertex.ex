defmodule Mic.Chat.Vertex do
  ...
  defp convert_message(%MicWeb.Message{sender: :assistant, content: content}),
    do: %{
      "role" => "model",
      "parts" => %{
        "text" => content
  ...
  defp convert_message(%MicWeb.Message{sender: :user, content: content}),
    do: %{
      "role" => "user",
      "parts" => %{
        "text" => content
  ...
  defp convert_message(%MicWeb.Message{sender: :system, content: content}),
    do: %{
      "role" => "user",
      "parts" => %{
        "text" => content
  ...
  def fix_messages(messages) do
    ...
    |> Enum.reduce([], fn message, acc ->
      case {List.last(acc), message} do
        ...
        {%MicWeb.Message{sender: prev_sender}, %MicWeb.Message{sender: :user}}
        when prev_sender in [:user, :system] ->
          acc ++ [%MicWeb.Message{sender: :assistant, content: "ok"}, message]

        # Insert assistant between user/system messages if consecutive
        {%MicWeb.Message{sender: :assistant}, %MicWeb.Message{sender: next_sender}}
        when next_sender in [:user, :system] ->
          acc ++ [message]

        # Insert user between assistant and assistant messages
        {%MicWeb.Message{sender: :assistant}, %MicWeb.Message{sender: :assistant}} ->
          acc ++ [%MicWeb.Message{sender: :user, content: "ok"}, message]
  ...
  def do_complete(messages, model, callback) do
    ...
    project_id = Application.get_env(:mic, :google_cloud_project_id, "")

    contents = messages |> fix_messages() |> Enum.map(&convert_message/1)
