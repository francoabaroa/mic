defmodule Mic.ArtistsTest do
  ...
  describe "profiles" do
    ...
    test "list_profiles/0 returns all profiles", %{user: user} do
      ...
      profiles = Artists.list_profiles()
              |> Enum.map(fn p -> Repo.preload(p, :user) end)
      assert profiles == [profile]
    end

    test "get_profile!/1 returns the profile with given id", %{user: user} do
      profile = profile_fixture(user)
      fetched_profile = Artists.get_profile!(profile.id) |> Repo.preload(:user)
    ...
    test "create_profile/1 with valid data creates a profile", %{user: user} do
      ...
      assert {:ok, %Profile{} = profile} = Artists.create_profile(user, valid_attrs)
    ...
    test "create_profile/1 with invalid data returns error changeset", %{user: user} do
      assert {:error, %Ecto.Changeset{}} = Artists.create_profile(user, @invalid_attrs)
    end
    ...
    test "update_profile/2 with invalid data returns error changeset", %{user: user} do
      ...
      assert profile == Artists.get_profile!(profile.id) |> Repo.preload(:user)
    end
